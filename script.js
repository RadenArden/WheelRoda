        ...
        function renderEntries() {
            ...
            names.forEach((name, index) => {
                ...
                entry.innerHTML = `
                    <span class="entry-drag">☰</span>
                    <input type="text" class="entry-input" value="${name}" data-index="${index}">
                    <button class="entry-delete" data-index="${index}">🗑️</button>
                `;
            ...
            document.querySelectorAll('.entry-input').forEach(input => {
                input.addEventListener('change', (e) => {
                    ...
                    names[index] = e.target.value.trim() || `Entry ${index + 1}`;
                    drawWheel();
                    updateBadges();
                });
            });
        ...
        function spinWheel() {
            ...
            const spins = 6;
            const targetRotation = currentRotation + spins * 360 + (360 - targetAngle) - (currentRotation % 360);
